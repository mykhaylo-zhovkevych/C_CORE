...
void Eingabe(int nr, adresse_t neu[]) {
   printf("Vorname : ");
...
void Suche(adresse_t search[], char buchstabe, int nr) {
   int i;
...
void Ausgabe(adresse_t all[],int nr) {
   int i;
...
void Sort(adresse_t sort[], int nr) {
   int i,j;
   adresse_t *temp;

   temp = (adresse_t*)malloc(sizeof(struct adres *));
   if(NULL == temp) {
